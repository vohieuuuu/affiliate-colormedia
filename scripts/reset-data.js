...
async function resetData() {
  ...
  try {
    ...
    for (const customer of customers1) {
      await storage.storage.addReferredCustomer(aff1.id, customer);
    }
    ...
    const affiliate2User = await storage.storage.createUser({
    ...
    const aff2 = await storage.storage.createAffiliate(aff2Data);
    ...
    for (const customer of customers2) {
      await storage.storage.addReferredCustomer(aff2.id, customer);
    }
