...
export default function KolWithdrawalModalV2({ 
  ...
  const [withdrawalDataForOtp, setWithdrawalDataForOtp] = useState<any>(null);
    ...
    onSuccess: (data) => {
      ...
      if (data.status === "success") {
        if (!data.data.exceeds) {
          ...
          // Set up data for OTP modal
          setWithdrawalDataForOtp({
            amount: amountValue,
            note: note || "",
            tax_id: taxId || ""
          });
  ...
  return (
    <>
      ...
      {withdrawalDataForOtp && (
        <KolWithdrawalOtpModal
          isOpen={showOtpModal}
          onClose={() => {
            setShowOtpModal(false);
            // Optionally re-open the first modal if user cancels OTP verification
            // setIsOpen(true);
          }}
          onSuccess={handleOtpSuccess}
          kolData={kolData}
          withdrawalData={withdrawalDataForOtp}
        />
      )}
    </>
  );
