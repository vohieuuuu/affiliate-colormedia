...
export default function KolWithdrawalOtpModal({ 
  ...
  // Mở modal -> mặc định gửi OTP
  useEffect(() => {
    if (isOpen && withdrawalData) {
      ...
      // Auto-send OTP when the modal is opened
      sendOtp();
    } else {
  ...
  const { mutate: sendOtp, isPending: isSendingOtp } = useMutation({
    mutationFn: async () => {
      console.log("Sending OTP for amount:", withdrawalData.amount, "note:", withdrawalData.note, "tax_id:", withdrawalData.tax_id);
